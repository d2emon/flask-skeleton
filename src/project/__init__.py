...
class Project():
    ...
    def __init__(self, appname="app", apppath="temp", **kwargs):
        ...
        self.github = kwargs.get('github', "https://github.com/%s/%s" % (self.account, self.appname))
        self.url = kwargs.get('url', "%s#readme" % (self.github))
        self.git_url = kwargs.get('git_url', "git+%s.git" % (self.github))
        self.isssues_url = kwargs.get('issues', "%s/issues" % (self.github))
